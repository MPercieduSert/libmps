 ...
 *\brief Interface mère des classes conditions d'unicité.
 */
template<class Ent> class AbstractUniqueSql_temp : public AbstractUniqueSql {
public:
    CONSTR_DEFAUT(AbstractUniqueSql_temp)

    //! Destructeur.
    ~AbstractUniqueSql_temp() override; //default
...
template<class Ent> AbstractUniqueSql_temp<Ent>::~AbstractUniqueSql_temp() {}

/*! \ingroup groupeUniqueSql
 *\brief Classe conditions d'unicité pour les entité ne possédant pas de condition d'unicité.
 */
template<class Ent> class NoUniqueSql : public AbstractUniqueSql_temp<Ent> {
public:
 ...
 */
template<class Ent> class SimpleUniqueSql : public AbstractUniqueSql_temp<Ent> {
protected:
    QString m_unique; //!< Requete Sql sur l'existence d'un ensemble d'attribus uniques.
    using AbstractUniqueSql_temp<Ent>::typeAttributSqlString;
    using AbstractUniqueSql_temp<Ent>::exec;
    using AbstractUniqueSql_temp<Ent>::finish;
    using AbstractUniqueSql_temp<Ent>::id;
    using AbstractUniqueSql_temp<Ent>::next;
    using AbstractUniqueSql_temp<Ent>::prepare;

public:
    using AbstractUniqueSql_temp<Ent>::uniqueString;
    using AbstractUniqueSql_temp<Ent>::uniqueCreerString;
...
protected:
    ...
    //! Execute la requète préparé puis teste l'identifiant de la requète avec l'entité.
    std::pair<bmps::existeUni,idt> execUniqueId(const Ent &ent) {
        std::pair<bmps::existeUni,idt> resultat(bmps::existeUni::Aucun,0);
        exec();
        if(next()) {
            resultat.second = id();
            if(entity.is_new() || entity.id() == resultat.second)
...
template<class Ent> bmps::existeUni SimpleUniqueSql<Ent>::existsUnique(Ent &ent) {
    prepare(m_unique);
    bindValuesUnique(entity);
    return execUnique(entity);
}

template<class Ent> std::pair<bmps::existeUni,idt> SimpleUniqueSql<Ent>::existsUniqueId(const Ent &ent) {
    prepare(m_unique);
    bindValuesUnique(entity);
...
protected:
    ...
    bmps::existeUni aucunAutreOuMeme(Ent &ent, idt idUni) const {
        if(idUni == 0)
            return bmps::Aucun;
        else {
            if(entity.is_new()) {
    ...
    std::pair<bmps::existeUni,idt> aucunAutreOuMemeId(const Ent &ent, idt idUni) const {
        if(idUni != 0)
            return std::pair<bmps::existeUni,idt>((entity.is_new() || entity.id() == idUni)?
                                                  bmps::existeUni::Tous :
    ...
    //! Modifie l'identifiant de l'entité si elle est nouvelle sinon teste les identifiants.
    bmps::existeUni autreOuMeme(Ent &ent, idt idUni) const {
        if(entity.is_new()) {
            entity.set_id(idUni);
            return bmps::Meme;
        }
    ...
    //! Teste les identifiants.
    std::pair<bmps::existeUni,idt> autreOuMemeId(const Ent &ent, idt idUni) const {
        return std::pair<bmps::existeUni,idt>((entity.is_new() || entity.id() == idUni)?
                                       bmps::existeUni::Meme :
                                       bmps::existeUni::Autre,
                                   idUni);
    ...
    virtual void bindValuesUnique_2(const Ent &ent) = 0;

    //! Retourne l'identifiant corespondant la seconde requête d'unicité.
    idt id_2Unique(const Ent &ent) {
        prepare(m_unique_2);
...
template<class Ent> bmps::existeUni DoubleUniqueSql<Ent>::existsUnique(Ent &ent)
{
    auto idUni1 = idUnique(entity);
    auto idUni2 = id_2Unique(entity);
    finish();
...
template<class Ent> std::pair<bmps::existeUni,idt> DoubleUniqueSql<Ent>::existsUniqueId(const Ent &ent)
{
    auto idUni1 = idUnique(entity);
    auto idUni2 = id_2Unique(entity);
    finish();
 ...
 */
template<class Ent> class relation_exact_one_not_nullUniqueSql : public virtual DoubleUniqueSql<Ent> {
protected:
...
public:
    enum {Id_1Unique = 0,NbrUnique_1,Id_2Unique = 0,NbrUnique_2,
          Id_1UniqueSet = DoubleUniqueSql<Ent>::UniqueSet1,
          Id_2UniqueSet = DoubleUniqueSql<Ent>::UniqueSet2
         };
    CONSTR_DEFAUT(relation_exact_one_not_nullUniqueSql)

    //! Destructeur.
    ~relation_exact_one_not_nullUniqueSql() override = default; //default
...
template<class Ent> bmps::existeUni relation_exact_one_not_nullUniqueSql<Ent>::existsUnique(Ent &ent) {
    if(entity.id_2() == 0) {
        if(entity.id_1() != 0) {
            prepare(m_unique);
            bindValuesUnique(entity);
...
template<class Ent> std::pair<bmps::existeUni,idt> relation_exact_one_not_nullUniqueSql<Ent>::existsUniqueId(const Ent &ent) {
    if(entity.id_2() == 0) {
        if(entity.id_1() != 0) {
            prepare(m_unique);
            bindValuesUnique(entity);
 ...
 */
template<class Ent> class Attsrelation_exact_one_not_nullUniqueSql : public relation_exact_one_not_nullUniqueSql<Ent> {
public:
    CONSTR_DEFAUT(Attsrelation_exact_one_not_nullUniqueSql)

    //! Destructeur.
    ~Attsrelation_exact_one_not_nullUniqueSql() override; //default

protected:
    //! Transmet les valeurs des attributs uniques aux requètes SQL préparées.
    virtual void bindValuesUniqueAtts(const Ent &ent) = 0;

    //! Transmet les valeurs des attributs uniques à la requète SQL préparée 1.
    void bindValuesUnique(const Ent &entity) override {
        relation_exact_one_not_nullUniqueSql<Ent>::bindValuesUnique(entity);
        bindValuesUniqueAtts(entity);
    }

    //! Transmet les valeurs des attributs uniques à la requète SQL préparée 2.
    void bindValuesUnique_2(const Ent &entity) override {
        relation_exact_one_not_nullUniqueSql<Ent>::bindValuesUnique_2(entity);
        bindValuesUniqueAtts(entity);
    }
};

template<class Ent> Attsrelation_exact_one_not_nullUniqueSql<Ent>::~Attsrelation_exact_one_not_nullUniqueSql() = default;
...
protected:
    ...
    using DoubleUniqueSql<Ent>::id_2Unique;
    using DoubleUniqueSql<Ent>::next;
    using DoubleUniqueSql<Ent>::prepare;
...
protected:
    //! Transmet les valeurs des attributs uniques à la requète SQL préparée 2.
    virtual void bindValuesUnique_3(const Ent &ent) = 0;

    //! Retourne l'identifiant corespondant la troisième requête d'unicité.
    idt id_3Unique(const Ent &ent) {
        prepare(m_unique_3);
...
template<class Ent> bmps::existeUni TripleUniqueSql<Ent>::existsUnique(Ent &ent) {
    auto idUni1 = idUnique(entity);
    auto idUni2 = id_2Unique(entity);
    auto idUni3 = id_3Unique(entity);
    finish();
...
template<class Ent> std::pair<bmps::existeUni,idt> TripleUniqueSql<Ent>::existsUniqueId(const Ent &ent) {
    auto idUni1 = idUnique(entity);
    auto idUni2 = id_2Unique(entity);
    auto idUni3 = id_3Unique(entity);
    finish();
 ...
 */
template<class Ent> class TripleExactOneNot_nullUniqueSql : public TripleUniqueSql<Ent>, relation_exact_one_not_nullUniqueSql<Ent> {
protected:
    using DoubleUniqueSql<Ent>::m_unique;
    using DoubleUniqueSql<Ent>::m_unique_2;
...
public:
    enum {Id_1Unique = 0, NbrUnique_1,
          Id_2Unique = 0, NbrUnique_2,
          id_3Unique = 0, NbrUnique_3,
          Id_1UniqueSet = TripleUniqueSql<Ent>::UniqueSet1,
          Id_2UniqueSet = TripleUniqueSql<Ent>::UniqueSet2,
          id_3UniqueSet = TripleUniqueSql<Ent>::UniqueSet3
         };
    CONSTR_DEFAUT(TripleExactOneNot_nullUniqueSql)

    //! Destructeur.
    ~TripleExactOneNot_nullUniqueSql() override = default; //default
...
template<class Ent> bmps::existeUni TripleExactOneNot_nullUniqueSql<Ent>::existsUnique(Ent &ent)
{
    if(entity.id_3() == 0)
        return relation_exact_one_not_nullUniqueSql<Ent>::existsUnique(entity);
    else if(entity.id_2() == 0 &&ent.id_1() == 0)
    {
        prepare(m_unique_3);
        bindValuesUnique_3(entity);
        return execUnique(entity);
...
template<class Ent> std::pair<bmps::existeUni,idt> TripleExactOneNot_nullUniqueSql<Ent>::existsUniqueId(const Ent &ent) {
    if(entity.id_3() == 0)
        return relation_exact_one_not_nullUniqueSql<Ent>::existsUniqueId(entity);
    else if(entity.id_2() == 0 &&ent.id_1() == 0) {
        prepare(m_unique_3);
        bindValuesUnique_3(entity);
        return execUniqueId(entity);
