 ...
 */
template<class Ent> class ManagerArbre : public virtual ManagerSql<Ent>
{
protected:
...
template<class Ent> void ManagerArbre<Ent>::deleteLeafOutOf(TreeItem<Ent> * tree)
{
    if(tree->hasChild())
    {
...
template<class Ent> void ManagerArbre<Ent>::getArbreRec(TreeItem<Ent> * tree)
{
    ListPtr<Arbre> childs = m_managerArbre.getList(Arbre::Parent,tree->data().id(),Arbre::Num);
    for(ListPtr<Arbre>::iterator i = childs.begin(); i != childs.end(); ++i)
    {
...
template<class Ent> void ManagerArbre<Ent>::save(Tree<Ent> & tree, bdd::TreeSave n)
{
    using namespace bdd;
...
template<class Ent> void ManagerArbre<Ent>::saveAddLeaf(TreeItem<Ent> * tree)
{
    int i = 0;
...
template<class Ent> void ManagerArbre<Ent>::saveWithoutDelete(TreeItem<Ent> * tree)
{
    int i = 0;
...
template<class Ent> void ManagerArbre<Ent>::saveExt(TreeItem<Ent> * tree, idt idRoot)
{
    int i = 0;
    auto idParent = tree->data().id();
    for(typename QList<TreeItem<Ent>*>::const_iterator child = tree->childs().cbegin(); child != tree->childs().cend(); ++i, ++child)
