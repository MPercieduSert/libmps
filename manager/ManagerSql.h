...
#define SAVE_VALIDE auto controle = bmps::None;\
    ...
    if(controle != bmps::None){ \
        if(controle != bmps::Conflit) { \
            ...
            entBdd.set_id(0); \
            getUnique(entBdd); \
            throw Uniqueentity_exception(QString("void ManagerSql<").append(name()).append(">::save_valide(") \
                                                       .append(name()).append("&)"),entity,entBdd);} \
        else throw Uniqueentity_exception(QString("void ManagerSql<").append(name()).append(">::save_valide(") \
                                        .append(name()).append("&)"),entity);}

// Macro pour manageur.
//! \ingroup groupeManager
//! Coprs des deux methodes save.
#define SAVE if(entity.is_valid()) \
    save_valide(entity); \
else \
    throw entities::invalide_entity_exception(QString("void ManagerSql<").append(name()).append(">::save(") \
                                                        .append(name()).append("&)"),entity);
...
public:
    ...
    //static const std::vector<QString> m_condition_nullString;
 ...
 */
template<class Ent> class ManagerSql : public AbstractManager_temp<Ent>, public AbstractManagerSql {
protected:
    LinkSql<Ent> m_link;            //!< Lien.
    InfoBdd m_info;                 //!< Information sur la table dans la base de donnée.
    std::unique_ptr<AbstractUniqueSql_temp<Ent>> m_unique;        //! Condition d'unicité.
    // Requête sql
    ...
    QString m_sqlGetListId_1Where1;    //!< Requéte sql de lecture des identifiants de la table vérifiant une condition triées suivant une colonne.
    ...
    QString m_sqlGetListId_1Where2;    //!< Requéte sql de lecture des identifiants de la table vérifiant une condition triées suivant deux colonnes.
    ...
    QString m_sqlGetListId_1Where3;    //!< Requéte sql de lecture des identifiants de la table vérifiant une condition triées suivant trois colonnes.
    ...
    QString m_sqlGetListId_2Where1;    //!< Requéte sql de lecture des identifiants de la table vérifiant deux conditions triées suivant une colonne.
    ...
    QString m_sqlGetListid_3Where1;    //!< Requéte sql de lecture des identifiants de la table vérifiant deux conditions triées suivant une colonne.
...
public:
    using AbstractManager_temp<Ent>::exists;
    using AbstractManager_temp<Ent>::existsUnique;
    using AbstractManager_temp<Ent>::del;
    using AbstractManager_temp<Ent>::get;
    using AbstractManager_temp<Ent>::getList;
    using AbstractManager_temp<Ent>::getUnique;
    using AbstractManager_temp<Ent>::name;
    using AbstractManager_temp<Ent>::sameInBdd;
    using AbstractManager_temp<Ent>::save;
    ...
    bool exists(const entity &ent) override {
        if(Ent::Verif_entity(entity)) {
            prepare(m_sqlExists);
            m_link.set_id(entity);
            exec();
            bool bb = next();
            finish();
            return bb;
            ...
            throw entities::invalide_entity_exception(QString("bool ManagerSql<").append(name()).append(">::exist(const entity &)"),entity);
    }
    ...
    bmps::existeUni existsUnique(Ent &ent) override
    ...
    bmps::existeUni existsUnique(const Ent &ent) override
        {return existsUniqueId(entity).first;}

    //! Teste s'il existe une entité ayant les mêmes valeurs d'attributs uniques que l'entité entity en base de donnée
    //! et renvoie l'identifiant du premier trouver.
    std::pair<bmps::existeUni,idt> existsUniqueId(const Ent &ent) override
    ...
    list_ptr<Ent> getList(const QString &condition) override {
    ...
    list_ptr<Ent> getList(typename Ent::Position ordre = Ent::Id, bool crois = true) override {
    ...
    list_ptr<Ent> getList(typename Ent::Position cle, const QVariant &value,
    ...
    list_ptr<Ent> getList(typename Ent::Position cle1, const QVariant &value1,
                         typename Ent::Position cle2,  const QVariant &value2,
                         typename Ent::Position ordre = Ent::Id,
    ...
    list_ptr<Ent> getList(const std::vector<typename Ent::Position> &cle, const std::vector<QVariant> &value,
    ...
    std::list<idt> getListId(typename Ent::Position cle, const QVariant &value,
                         typename Ent::Position ordre = Ent::Id,
                         bmps::condition cond = bmps::condition::Egal, bool crois = true) override {
        prepare(m_sqlGetListId_1Where1.arg(attribut(cle),m_conditionString[cond],
                                        attribut(ordre),croissant(crois)));
        bindValue(0,value);
    ...
    std::list<idt> getListId(typename Ent::Position cle, const QVariant &value,
        ...
        prepare(m_sqlGetListId_1Where2.arg(attribut(cle), m_conditionString[cond],
                                        attribut(ordre1), croissant(crois1),
                                        attribut(ordre2), croissant(crois2)));
        bindValue(0,value);
        return listIdFromRequete();
    ...
    std::list<idt> getListId(typename Ent::Position cle, const QVariant &value,
                           ...
                           bmps::condition cond = bmps::condition::Egal,
                           bool crois1 = true, bool crois2 = true, bool crois3 = true) override {
        prepare(m_sqlGetListId_1Where3.arg(attribut(cle), m_conditionString[cond],
                                        attribut(ordre1), croissant(crois1),
                                        attribut(ordre2), croissant(crois2),
                                        attribut(ordre3), croissant(crois3)));
        bindValue(0,value);
    ...
    std::list<idt> getListId(typename Ent::Position cle1, const QVariant &value1,
                           ...
                           bmps::condition cond1 = bmps::condition::Egal, bmps::condition cond2 = bmps::condition::Egal,
                           bool crois = true) override {
        prepare(m_sqlGetListId_2Where1.arg(attribut(cle1), m_conditionString[cond1],
                                        attribut(cle2), m_conditionString[cond2],
                                        attribut(ordre), croissant(crois)));
        bindValue(0,value1);
        bindValue(1,value2);
    ...
    std::list<idt> getListId(typename Ent::Position cle1, const QVariant &value1,
                         ...
                         bmps::condition cond3 = bmps::condition::Egal,
                         bool crois = true) override {
        prepare(m_sqlGetListid_3Where1.arg(attribut(cle1), m_conditionString[cond1],
                                        attribut(cle2), m_conditionString[cond2],
                                        attribut(cle3), m_conditionString[cond3],
                                        attribut(ordre), croissant(crois)));
        bindValue(0,value1);
    ...
    list_ptr<Ent> getListJoin(const QString &tableJoin, post colonneTable,
    ...
    list_ptr<Ent> getListJoin(const QString &tableJoin,
                             const QString &colonneJoin,
    ...
    //! ordonnée suivant la colonne de l'entité d'identifiant ordre.
    list_ptr<Ent> getListJoin(const QString &tableJoin1,
    ...
    map_id<Ent> getMap(const QString &condition, typename Ent::Position cleMap = Ent::Id) {
        prepare(m_sqlGetListWhere.arg(condition));
        return mapFromRequete(cleMap);
    }

    //! Renvoie la map des entités de la table.
    map_id<Ent> getMap(typename Ent::Position cleMap = Ent::Id) {
    ...
    //! Renvoie la map des entités de la table vérifiant la condition,
    //! valeur de la colonne d'identifiant cle = value.
    map_id<Ent> getMap(typename Ent::Position cle, const QVariant &value,
    ...
    //! valeur de la colonne d'identifiant cle1 = value1 et valeur de la colonne d'identifiant cle2 = value2.
    map_id<Ent> getMap(typename Ent::Position cle1, const QVariant &value1,
                       typename Ent::Position cle2,  const QVariant &value2,
    ...
    //! ordonnée suivant les colonnes d'identifiant contenue dans ordre,
    //! croissante (crois[i]=true) ou décroissante (croiss[i]=false).
    map_id<Ent> getMap(const std::vector<typename Ent::Position> &cle, const std::vector<QVariant> &value,
                       typename Ent::Position cleMap = Ent::Id,
                       const std::vector<typename Ent::Position> &ordre = std::vector<typename Ent::Position>(),
                       const std::vector<bmps::condition> &condition = std::vector<bmps::condition>(),
                       const std::vector<bool> &crois = std::vector<bool>()) override {
    ...
    //! valeur des colonnes de la table Ent d'identifiant key = value de std::map whereMapTable,
    //! valeur des colonnes de la table Join key = value de std::map whereMapJoin.
    map_id<Ent> getMapJoin(const QString &tableJoin, post colonneTable,
                           const QString &colonneJoin,
                           const std::map<typename Ent::Position,QVariant> &whereMapTable,
                           const std::map<QString,QVariant> &whereMapJoin,
    ...
    //! valeur de la colonne de la jointure d'identifiant cleWhere = valueWhere.
    map_id<Ent> getMapJoin(const QString &tableJoin,
                           const QString &colonneJoin,
                           const QString &whereJoin,
                           const QVariant &valueWhere,
                           typename Ent::Position cleMap = Ent::Id,
    ...
    //! Retourne True si l'opération s'est correctement déroulée.
    bool getUnique(Ent &ent) override {
    ...
    bool sameInBdd(const Ent &ent) override {
    ...
    bool sameInBdd(const Ent &ent, idt id) override {
        Ent entityT(id);
        if(!get(entityT))
            return false;
        entityT.set_id(entity.id());
    ...
    void saveUnique(Ent &ent) override {
        if(entity.is_valid()) {
            auto id = entity.id();
            bmps::existeUni n = existsUnique(entity);
            if(n <= bmps::existeUni::Meme) {
                ...
                throw Uniqueentity_exception(QString("void ManagerSql<").append(name()).append(">::saveUnique(") \
                                            .append(name()).append("&)"), entity);
        }
        else
            throw entities::invalide_entity_exception(QString("void ManagerSql<").append(name()).append(">::saveUnique(") \
                                                                                       .append(name()).append("&)"),entity);
    }
    ...
    //! soit entity à un identifiant idE non nul alors l'entité d'identifiant idU est mise à jour
    //! et l'entité d'identifiant idE est supprimé.
    void saveUnique(const Ent &ent) override {
        if(entity.is_valid())
        {
            ...
            if(pairExists.first <= bmps::existeUni::Meme) {
                ...
                if(!entity.is_new() &&pairExists.second != entity.id())
                    del(entity.id());
            }
            else
                throw Uniqueentity_exception(QString("void ManagerSql<").append(name()).append(">::saveUnique(const ") \
                                            .append(name()).append("&)"),entity);
        }
        else
            throw entities::invalide_entity_exception(QString("void ManagerSql<").append(name()).append(">::saveUnique(const ") \
                                                                                       .append(name()).append("&)"),entity);
    }
...
protected:
    ...
    map_id<Ent> mapFromRequete(typename Ent::Position cle) {
        exec();
        map_id<Ent> map;
        while(next())
...
template<class Ent> ManagerSql<Ent>::ManagerSql(const InfoBdd &info, std::unique_ptr<AbstractUniqueSql_temp<Ent>> &&unique)
    : m_info(info),
    ...
    //{
        ...
        for(post i = 1; i != m_info.nbr_att(); ++i) {
            ...
            if(m_info.creerAttribut(i).second)
                sql.append(" ").append(wordSqlString(bmps::wordSql::Not_null));
        }
...
template<class Ent> QString ManagerSql<Ent>::messageErreurs(const entity &ent) const {
...
template<class Ent> void ManagerSql<Ent>::writeStringSql() {
    ...
    m_sqlGetListId_1Where1.append(m_sqlGetListIdWhereOrderby.arg("%1%2","%3%4"));
    m_sqlGetListId_1Where1.squeeze();
    ...
    m_sqlGetListId_1Where2.append(m_sqlGetListId_1Where1);
    m_sqlGetListId_1Where2.append(",%5%6");
    m_sqlGetListId_1Where2.squeeze();
    ...
    m_sqlGetListId_1Where3.append(m_sqlGetListId_1Where2);
    m_sqlGetListId_1Where3.append(",%7%8");
    m_sqlGetListId_1Where3.squeeze();
    ...
    m_sqlGetListId_2Where1.append(m_sqlGetListIdWhereOrderby.arg("%1%2? AND %3%4","%5%6"));
    m_sqlGetListId_2Where1.squeeze();
    ...
    // Get list 3 where 1 ordre
    m_sqlGetListid_3Where1.append(m_sqlGetListIdWhereOrderby.arg("%1%2? AND %3%4? AND %5%6","%7%8"));
    m_sqlGetListid_3Where1.squeeze();
