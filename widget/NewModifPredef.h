 ...
 */
class EvenementNewModifForm : public AbstractTypenc_nomNewModifForm {
    Q_OBJECT
protected:
...
public:
    ...
    bool valide() const override {
        if(!AbstractTypenc_nomNewModifForm::valide() || m_titreEdit->text().isEmpty())
            return false;
 ...
 *\brief Formulaires de création d'un style d'événements.
 */
class evenement_styleNewModifForm : public AbstractNomNewModifForm {
    Q_OBJECT
protected:
    //! Objet .
    enum objet {Bordure,
 ...
 */
class SourceNewModifForm : public dialogMPS::AbstractTypenc_nomNewModifForm {
    Q_OBJECT
protected:
