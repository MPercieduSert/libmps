...
\footnotesize
	...
	\tableEntity{DansClasse}{
		...
		\fkey{idEtab}{Etablissement}\\
		\fkey{idNiveau}{Niveau}\\
		\unique{idEtab, idNiveau, num}
	}[left=1.5cm of DansClasse]

	\tableEntity{GroupePour}{
		idAn&int&idAn&NULL\\
		idClasse&int&idCl&NULL\\
		idGroupe&int&idGr
	}{
		\fkey{idAn}{Annee}\\
		\fkey{idClasse}{Classe}\\
	...
	\tableEntity{DansControle}{
		...
		idEnonce&int&idEn\\
		idClasse&int&idCl&NULL\\
		idEleve&int&idEl&NULL\\
		idGroupe&int&idGr&NULL\\
		num&int&num&NULL
	}{
		\fkey{idControle}{Controle}\\
	...
	\tableEntity{Enonce}{
		idControle&int&idCtr&NULL\\
		type&int&tp\\
		version&int&ver\\
		ARBRE & ENONCE&
	}{
		\fkey{id}{ArbreEnonce}\\
		\fkey{idContole}{Controle}
