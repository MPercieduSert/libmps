...
enum roleNode : flag::flag_type {
    ...
    Date_TimeRole = 0x100000,            //!< Donnée principale sous forme de date et horaire (QDateTime)
    ...
    MainAllRole = StringRole | CheckStateRole | DateRole | Date_TimeRole | TimeRole | IntRole | NumRole
...
public:
    ...
    virtual flag set_data(int /*cible*/, const QVariant &/*value*/, int /*role*/, numt /*num*/ = 0) {return NoRole;}
...
public:
    ...
    NodeIndex first() const noexcept;
...
public:
    ...
    void erase(const NodeIndex &index, numt count)
        {m_tree.erase(index.m_iter,index.iter().to_brotherU(count));}
...
public:
    ...
    bool set_data(const NodeIndex &index, const QVariant &value, int role) {
        Change change;
        change.type = SetData;
...
protected:
    ...
    void beginReset_data()
        {emit modelAboutToReset_data();}
...
template<class U, class F>  void TreeForNodeModel::setTree(const conteneurMPS::tree<U> &t, F usine) {
    ...
    while (iterU) {
        if(!iterU.leaf())
            for (auto i = iterU.cbegin_child(); i; ++i)
